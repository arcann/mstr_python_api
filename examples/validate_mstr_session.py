...
def validate_session(session_state):
    ...
    if session_state is None:
        ...
        try:
            ...
            mstr_client = TaskProc(base_url=base_url, session_state=session_state)
            mstr_client.trace = True
            privs = mstr_client.check_user_privileges()
            authorized = True
...
if __name__ == '__main__':
    ...
    if session_state is None:
        mstr_client = TaskProc(base_url=base_url,
                               ...
                               username=user_name,
                               password=sys.argv[1])
        session_state = mstr_client.session
        logout_session = True

    log.info("validation_session")
    validate_session(session_state)

    if logout_session and mstr_client is not None:
